...
class BaseEMO:
    ...
    def _make_offspring(self, parent_X, parent_F, parent_CV):
        """Binary tournament + SBX + poly mutation -> N offspring."""
        N = self.N
        Q_X = np.zeros((N, self.n_var))
        for i in range(0, N, 2):
            i1 = self._tournament(parent_F, parent_CV)
            i2 = self._tournament(parent_F, parent_CV)
            c1, c2 = sbx_crossover(
                parent_X[i1], parent_X[i2],
                self.xl, self.xu, self.eta_c, self.pc)
            c1 = polynomial_mutation(c1, self.xl, self.xu, self.eta_m)
            c2 = polynomial_mutation(c2, self.xl, self.xu, self.eta_m)
            Q_X[i] = c1
            if i + 1 < N:
                Q_X[i + 1] = c2
    ...
    def run(self):
        ...
        while self.fe_count < self.max_fe:
            ...
            for i in range(self.N):
                ...
                w_i = self.W[i]
                dists_to_i = np.array([
                    np.linalg.norm(self.W[j] - w_i) for j in range(self.N)
                ])
                nb = np.argsort(dists_to_i)[:self.T]
...
class CNSGAIII(BaseEMO):
    ...
    def _constrained_non_dominated_sort(self, F, CV):
        """Non-dominated sort using constrained dominance."""
        N = len(F)
        S = [[] for _ in range(N)]
        ...
        for p in range(N):
            for q in range(N):
                if p == q:
                    continue
                if self._constrained_dominates(F[p], CV[p], F[q], CV[q]):
                    S[p].append(q)
                elif self._constrained_dominates(F[q], CV[q], F[p], CV[p]):
                    n_dom[p] += 1

        fronts = []
        current = [p for p in range(N) if n_dom[p] == 0]
    ...
    def _c_nds(self, F, CV):
        ...
        for p in range(N):
            for q in range(N):
                if p == q:
                    continue
                if self._c_dom(F[p], CV[p], F[q], CV[q]):
                    S[p].append(q)
                elif self._c_dom(F[q], CV[q], F[p], CV[p]):
                    n_dom[p] += 1
