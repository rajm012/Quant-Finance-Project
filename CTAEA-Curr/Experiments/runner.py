...
# Default budget fractions for C-TAEA per-stage IGD/HV (10%, …, 100%)
DEFAULT_CTAEA_METRIC_STAGES = tuple(round(i * 0.1, 10) for i in range(1, 11))
...
def run_single(problem_name, m, algo_name, seed=0, verbose=False,
               collect_metrics_stages=None):
    """
    Run one algorithm instance on one problem/dimension combination.

    Parameters
    ----------
    collect_metrics_stages : sequence of float or None
        For ``C-TAEA`` only: fractions of max_fe at which IGD/HV are stored in
        ``metrics_trace``. Use ``DEFAULT_CTAEA_METRIC_STAGES`` or a custom tuple.

    Returns
    -------
    dict with keys: igd, hv, n_feasible, time_sec, F_nd, and optionally metrics_trace
    """
    ...
    if algo_name == 'C-TAEA':
        ...
        if collect_metrics_stages:
            extra['collect_metrics_stages'] = collect_metrics_stages
            extra['hv_ref_point'] = get_ref_point(problem_name, m)
        algo = AlgoClass(
            problem=problem,
            N=N,
            max_fe=max_fe,
    ...
    if algo_name == 'C-TAEA' and getattr(algo, 'metrics_trace', None):
        out['metrics_trace'] = algo.metrics_trace
    return out
...
def _run_single_safe(task):
    ...
    if len(task) == 7:
        run_idx, problem_name, m, algo_name, seed, verbose, cms = task
    else:
...
def run_experiment(problem_names=None, m_values=None, algo_names=None,
    ...
    collect_metrics_stages : sequence or True or None
        If a sequence, passed to C-TAEA for per-run ``metrics_trace``.
        If True, uses ``DEFAULT_CTAEA_METRIC_STAGES``.
    """
    ...
    if collect_metrics_stages is True:
        collect_metrics_stages = DEFAULT_CTAEA_METRIC_STAGES
        ...
        for m in m_values:
            ...
            for algo_name in algo_names:
                ...
                for run_idx in range(n_runs):
                    ...
                    cms = (collect_metrics_stages if algo_name == 'C-TAEA'
                           and collect_metrics_stages else None)
                    if cms is not None:
                        tasks.append((run_idx, prob_name, m, algo_name, seed, verbose, cms))
                    else:
