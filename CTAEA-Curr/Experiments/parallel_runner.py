...
class TaskConfig:
    ...
    run_idx: int
    seed: int
    collect_metrics_stages: Optional[Tuple[float, ...]] = None
...
class TaskResult:
    ...
    metrics_trace: Optional[List[Dict[str, Any]]] = None
...
def run_single_task(task: TaskConfig) -> TaskResult:
    ...
    try:
        ...
        if task.algo_name == 'C-TAEA':
            extra = {}
            if task.collect_metrics_stages:
                val = ZR_TABLE_LOCAL.get(task.problem_name, ZR_TABLE_LOCAL['default'])
                extra['collect_metrics_stages'] = task.collect_metrics_stages
                extra['hv_ref_point'] = np.full(task.m, val)
            algo = AlgoClass(
                problem=problem,
                N=N,
                max_fe=max_fe,
        ...
        mtrace = None
        if task.algo_name == 'C-TAEA' and getattr(algo, 'metrics_trace', None):
            mtrace = list(algo.metrics_trace)
...
def run_experiment_parallel(
        ...
        for m in m_values:
            ...
            for algo_name in algo_names:
                for run_idx in range(n_runs):
                    ...
                    cms = (collect_metrics_stages
                           if algo_name == 'C-TAEA' and collect_metrics_stages
                           else None)
                    tasks.append(TaskConfig(
                        prob_name, m, algo_name, run_idx, seed,
        ...
        for future in as_completed(future_to_idx):
            ...
            try:
                ...
                results.append(TaskResult(
                    ...
                    time_sec=0.0, F_nd=[], error=str(e), metrics_trace=None,
                ))
    ...
    for (prob, m, algo), group in grouped.items():
        ...
        data = {
            ...
            'runs': [
                {
                    'run_idx': r.task.run_idx,
                    'igd': r.igd,
                    'hv': r.hv,
                    'n_feasible': r.n_feasible,
                    'time_sec': r.time_sec,
                    **({'metrics_trace': r.metrics_trace} if r.metrics_trace else {}),
                }
                for r in group
            ],
        }
...
def main():
    ...
    parser.add_argument('--trace-metrics', action='store_true',
                        help='Record C-TAEA IGD/HV at 10%%–100%% FE (larger JSON)')
    ...
    cms = True if args.trace_metrics else None
    if args.quick:
        run_experiment_parallel(
            quick_test=True, n_workers=args.workers, output_dir=args.output,
            collect_metrics_stages=cms,
        )
    elif args.full:
        run_experiment_parallel(
            n_workers=args.workers, output_dir=args.output,
            collect_metrics_stages=cms,
        )
    else:
