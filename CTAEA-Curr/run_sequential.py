...
def run_single_model(algo_name, problems, m_values, n_runs, workers, output_dir,
                     resume=True, trace_metrics=False):
    """
    ...
    try:
        results = run_experiment_parallel(
            ...
            verbose=True,
            collect_metrics_stages=(True if trace_metrics else None),
        )
...
def run_all_sequential(problems, m_values, n_runs, workers, output_dir, resume=True,
                       model_order=None, trace_metrics=False):
    """
...
Examples:
  ...
  python run_sequential.py --all --problems C1DTLZ1 C1DTLZ3 --m-values 3 5
    ...
    parser.add_argument('--trace-metrics', action='store_true',
                        help='Record C-TAEA per-stage IGD/HV in JSON (slower)')
